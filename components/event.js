			...
			{({ inView, ref }) => {
				...
				return (
					<div ref={ref} className='mt-12 md:mt-24 bg-primary'>
						<div className={`${animation2(50)} container mx-auto py-16 grid grid-cols-2 gap-16`}>
							<div>
								<h3 className='font-bold text-6xl text-secondary'>{event?.fields?.name}</h3>
								<h3 className='font-thin text-4xl text-secondary mt-4'>{event?.fields?.date} @ {event?.fields?.startTime}</h3>
								<h3 className='font-thin text-2xl text-secondary mt-8'> {event?.fields?.eventLocation} </h3>
								<h3 className='font-thin text-2xl text-secondary mt-4'> {event?.fields?.eventAddress} </h3>
							...
							<div>
								<h3 className='font-bold text-4xl text-secondary text-right'>Featuring</h3>
								{event?.fields?.preformers?.map((performer) => {
									const artist = FindEntryBySysId(performer?.sys?.id, preformers)
									return (
										<h3 className='font-thin text-2xl text-secondary text-right mt-3'>{artist?.fields?.name}, {artist?.fields?.instrument}</h3>
									)
								})}
								
								<h3 className='font-bold text-4xl text-secondary text-right mt-8'>Program Details</h3>
